...
let preamble _ comment _ usf =
  ...
  str "#lang rosette\n" ++
  str "(require \"macros_extr.rkt\")\n\n" ++
  str "(provide (all-defined-out))\n\n" ++
...
let prlist_pref_sep sep_thunk lastsep_thunk pref_thunk elem l =
  let sep = sep_thunk () in
  let lastsep = lastsep_thunk () in
  let elems = List.map elem l in
  let pref = pref_thunk () in
  let filtered_elems =
  ...
  let rec insert_seps es =
    ...
    | []     -> mt ()
    | [e]    -> pref ++ e
    | h::[e] -> pref ++ h ++ lastsep ++ pref ++ e
    | h::t   -> pref ++ h ++ sep ++ insert_seps t
  in
...
and pp_one_pat env (ids,p,t) =
  ...
  let args =
    ...
    else (str " " ++ prlist_pref_sep spc spc (fun x -> str ",") pr_id (List.rev ids))
  in
